 ...
 *
 * CS344 Assignment 4: OTP (otp_enc.c)
 *
 * read in plaintext and a key given from the command line
 *
 * make a socket connection to
 * 	otp_enc_d.c, via a port given from the command line
 * 	the connection will return cipher_text
 * ********************************************************/
...
#define ROW_LENGTH 10 //Letters read at a time
...
void fill_buffer(char** buffer[2], char* plaintext_file, char* key_file);
...
{	
	...
	if(misc_buffer[0] == 'N')
	{
		printf("Error: otp_enc could not contact otp_enc_d on port %s\n", port);
		return 2;
	}
 ...
 *		from plaintext_file and key_file
 *
 * ********************************************************/
void fill_buffer(char ** buffer[2], char* plaintext_file, char* key_file)
{

	read_from_file(buffer[0], plaintext_file);
