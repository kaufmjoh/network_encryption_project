 ...
 *
 * CS344 Assignment 4: OTP (otp_dec_d)
 *
 ...
 *
 * When a connection is made, this daemon will read from the client ciphertext
 * and a key, decrypt it, and write back to the client plaintext
 ******************************* ***************************************/
...
void dec_message(char* ciphertext, char* key, char* decrypted, int size);
 ...
 * message, ciphertext, and a key. This function writes plaintext back
 * to the client
 * ************************************************************************/
void commence_communications(int establishedConnectionFD, char** buffer[3])
{
...
//	sleep(5);
	...
	//DECRYPT CIPHERTEXT
	//printf("SERVER: Preparing to decrypt message\n");
	i = 0;
	while(i < (size / ROW_LENGTH)+1)
	{
		dec_message(buffer[0][i], buffer[1][i], buffer[2][i], size);
	//	printf("SERVER: created an decrypted line: %s\n", buffer[2][i]);
		i++;
	}

	//SEND the plaintext to the Client
	i = 0;
	while(i < (size / ROW_LENGTH) + 1)
	{
 ...
 * void dec_message(char* ciphertext, char* key, char* decrypted, int size)
 *
 *	Decrypt the line of ciphertext with the key, storing results in
 *	decrypted.
 *
...
void dec_message(char* ciphertext, char* key, char* decrypted, int size)
{
	...
	while(i < ROW_LENGTH) //for char in the row
	{
		...
		if((ciphertext[i] < 65 || ciphertext[i] > 90) && ciphertext[i] != ' ' &&ciphertext[i] != '\0')
		{
